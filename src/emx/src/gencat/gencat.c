...
#else
		    if ((ofd = open(catfile, O_WRONLY|O_TRUNC|O_CREAT|O_BINARY, 0666)) < 0)
#endif
...
#else
		} else if ((ofd = open(catfile, O_RDONLY|O_BINARY)) < 0) {
		    if ((ofd = open(catfile, O_WRONLY|O_CREAT|O_BINARY, 0666)) < 0)
#endif
...
#else
		    if ((ofd = open(catfile, O_WRONLY|O_TRUNC|O_BINARY)) < 0)
#endif
...
#else
		if ((ifd = open(input, O_RDONLY|O_TEXT)) < 0)
#endif
...
writeIfChanged(char *fname, int lang, int orConsts)
{
    ...
    if (!tmp)
        tmp = "/tmp";
    strcat(strcpy(tmpname, tmp), "/gencat.XXXXX");
#endif
...
#else
	if ((fd = open(fname, O_WRONLY|O_CREAT|O_BINARY, 0666)) < 0)
#endif
...
#else
	if ((fd = open(fname, O_WRONLY|O_TRUNC|O_BINARY)) < 0)
#endif
