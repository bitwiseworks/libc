/* $Id: SafeDosReadQueue.c 3943 2014-12-28 23:31:07Z bird $ */
/** @file
 * kLibC - Safe DosReadQueue().
 *
...
ULONG APIENTRY SafeDosReadQueue(HQUEUE hq, PREQUESTDATA pRequest, PULONG pcbData, PPVOID ppvData, ULONG ulElement,
                                ...
                                BOOL32 fNowait, PBYTE pbPriority, HEV hsem)
{
    ...
    rc = DosReadQueue(hq, SAFE_INOUTTYPE_USE(pRequest), SAFE_INOUTTYPE_USE(pcbData), SAFE_INOUTTYPE_USE(ppvData),
                      ulElement, fNowait, SAFE_INOUTTYPE_USE(pbPriority), hsem);
