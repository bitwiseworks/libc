 ...
 * 
 * @param   pszTag      If NULL the returned value is strtof("NAN", NULL).
 *                      Other wise the returned value is strtof("NAN(pszTag)", NULL).
 */
float _STD(nanf)(const char *pszTag)
{
    float rc;
    if (!pszTag)
    {
        static int volatile     s_fCacheValid = 0;
        static float volatile   s_rfCached;
        if (s_fCacheValid)
            rc = s_rfCached;
        else
        {
