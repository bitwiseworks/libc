/* $Id: setruid.c 2254 2005-07-17 12:25:44Z bird $ */
/** @file
 *
 * LIBC - setruid().
 *
 ...
 * @param   rgid    The new real user id.
 */
int _STD(setruid)(uid_t ruid)
{
    LIBCLOG_ENTER("ruid=%d (%#x)\n", ruid, ruid);
    int rc = __libc_Back_processSetUidAll(ruid, -1, -1);
    if (!rc)
