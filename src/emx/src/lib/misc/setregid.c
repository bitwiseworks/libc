/* $Id: setregid.c 2254 2005-07-17 12:25:44Z bird $ */
/** @file
 *
 * LIBC - setregid().
 *
...
/**
 * Sets the real group id and the effective group id of the current process.
 *
 ...
 */
int _STD(setregid)(gid_t rgid, gid_t egid)
{
    LIBCLOG_ENTER("rgid=%d (%#x) egid=%d (%#x)\n", rgid, rgid, egid, egid);
    int rc = __libc_Back_processSetGidAll(rgid, egid, -1);
    if (!rc)
