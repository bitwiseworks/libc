/* $Id: chown.c 3817 2014-02-19 01:40:34Z bird $ */
/** @file
 * chown()
 */
 ...
 * @param   pszPath Path to the file to change owner/group of, all symbolic
 *                  links are followed.
 ...
 */
int	 _STD(chown)(const char *pszPath, uid_t uid, gid_t gid)
{
    LIBCLOG_ENTER("pszPath=%p:{%s} uid=%ld gid=%ld\n", (void *)pszPath, pszPath, (long)uid, (long)gid);
    int rc = __libc_Back_fsFileOwnerSet(pszPath, uid, gid);
    if (!rc)
