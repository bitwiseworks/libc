/* $Id: DosOpenEventSemEx.c 2254 2005-07-17 12:25:44Z bird $ */
/** @file
 *
 * DosOpenEventSemEx.
 *
...
{
    LIBCLOG_ENTER("pszName=%p:{'%s'} phev=%p:{%#lx}\n", pszName, pszName, (void *)phev, *phev);
    PDOSEX      pDosEx;
    HEV         hev;
    int         rc;
    FS_VAR();
     ...
     */
    hev = *phev;
    pDosEx = __libc_dosexFind(DOSEX_TYPE_EVENT_OPEN, hev);
    if (pDosEx)
    {
#ifdef PER_PROCESS_OPEN_COUNTS
        pDosEx->u.EventOpen.cOpens++;
#endif
    ...
    else
    {
         ...
         */
        pDosEx = __libc_dosexAlloc(DOSEX_TYPE_EVENT_OPEN);
        if (!pDosEx)
        {
