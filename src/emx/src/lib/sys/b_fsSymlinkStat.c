/* $Id: b_fsSymlinkStat.c 2254 2005-07-17 12:25:44Z bird $ */
/** @file
 *
 * LIBC SYS Backend - lstat.
 *
...
{
    LIBCLOG_ENTER("pszPath=%p:{%s} pStat=%p\n", (void *)pszPath, pszPath, (void *)pStat);
    ...
    if (!rc)
        rc = __libc_back_fsNativeFileStat(szNativePath, pStat);
