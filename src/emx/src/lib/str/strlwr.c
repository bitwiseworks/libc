...
static int __uni_strlwr (UniChar *ucs, void *arg)
{
  ...
  (void)arg;
  for (c = ucs; *c; c++)
    *c = UniTransLower (__libc_GLocaleCtype.lobj, *c);
  return 1;
}
...
char *_STD(strlwr) (char *string)
{
  ...
  if (__libc_GLocaleCtype.mbcs)
  {
    while ((c = *s))
    {
      ...
      {
        /* Alright, we encountered a MBCS character. Convert everything
           until the end to Unicode, do the work in Unicode and then
           convert back to MBCS. */
        FS_VAR();
        FS_SAVE_LOAD();
        __libc_ucs2Do (__libc_GLocaleCtype.uobj, (char *)s, s, __uni_strlwr);
        FS_RESTORE();
        break;
      }
      *s++ = __libc_GLocaleCtype.auchLower [c];
    }
  }
  else
    while ((c = *s))
