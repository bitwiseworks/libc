...
#ifdef __EMX__
static _fmutex	arc4random_mtx = _FMUTEX_INITIALIZER;
#else
...
#ifdef __EMX__
#define	THREAD_LOCK()   _fmutex_request(&arc4random_mtx, 0)
#define	THREAD_UNLOCK()	_fmutex_release(&arc4random_mtx)
#else /* !__EMX__ */
#define	THREAD_LOCK()						\
	do {							\
		if (__isthreaded)				\
