...
#ifdef __EMX__
extern _fmutex __libc_gdtoa_locks[2];
#define ACQUIRE_DTOA_LOCK(n)	do {				\
    if (__libc_gdtoa_locks[n].fs == _FMS_UNINIT)                     \
        _fmutex_checked_create2(&__libc_gdtoa_locks[n], 0, "gdtoa lock no." #n); \
    _fmutex_checked_request(&__libc_gdtoa_locks[n], 0);              \
} while(0)
#define FREE_DTOA_LOCK(n)	do {				\
    _fmutex_checked_release(&__libc_gdtoa_locks[n]);                 \
} while(0)
#else /* !__EMX__ */
