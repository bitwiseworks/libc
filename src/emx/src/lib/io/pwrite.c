/* $Id: pwrite.c 2254 2005-07-17 12:25:44Z bird $ */
/** @file
 *
 * pwrite.
 *
...
/**
 * Write to a file without changing the file pointer.
 *
 ...
 * @param   off     Where in the file to write it.
 */
ssize_t	_STD(pwrite)(int fh, const void *pvBuf, size_t cbBuf, off_t off)
{
    ...
    if (offSaved >= 0)
    {
        ...
        if (lseek(fh, off, SEEK_SET) >= 0)
        {
             ...
             * Attemp the write.
             */
            ssize_t cbWritten = write(fh, pvBuf, cbBuf);
            if (cbWritten >= 0)
            {
                /*
