/* $Id: lutimes.c 2254 2005-07-17 12:25:44Z bird $ */
/** @file
 *
 * LIBC - lutimes.
 *
...
/**
 * Sets the access and modification times of a symbolic link or file.
 *
 ...
 */
int _STD(lutimes)(const char *pszPath, const struct timeval *paTimes)
{
    LIBCLOG_ENTER("pszPath=%p:{%s} paTimes=%p:{{.tv_sec=%ld, .tv_usec=%ld}, {.tv_sec=%ld, .tv_usec=%ld}}\n",
    ...
    int rc = __libc_Back_fsSymlinkTimesSet(pszPath, paTimes);
    if (!rc)
