/* $Id: _hrealloc.c 2254 2005-07-17 12:25:44Z bird $ */
/** @file
 *
 * High Memory Heap - realloc().
 *
...
/**
 * Rellocate a memory block from the high memory heap.
 *
 ...
 */
void *_hrealloc(void * pv, size_t cbNew)
{
    LIBCLOG_ENTER("pv=%p cbNew=%d\n", pv, cbNew);
    void *pvRet;
    if (!pv)
        pvRet = _hmalloc(cbNew);
    else
