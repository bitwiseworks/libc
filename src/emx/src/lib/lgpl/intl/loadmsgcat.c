...
#else
# define __libc_lock_define_initialized_recursive(a,b)  do {} while (0)
# define __libc_lock_lock_recursive(l)                  do {} while (0)
# define __libc_lock_unlock_recursive(l)                do {} while (0)
#endif
     ...
     const char *name;
{
  ...
  if (name[0] == 'I' && name[1] == '\0')
    {
#if defined _LIBC || (defined (__GLIBC__) && (__GLIBC__ > 2 || (__GLIBC__ == 2 && __GLIBC_MINOR__ >= 2)))
...
#   if (defined (__GLIBC__) && ( (__GLIBC__ == 2 && __GLIBC_MINOR__ >= 2) || __GLIBC__ > 2)) \
       || (defined (_LIBICONV_VERSION) && _LIBICONV_VERSION >= 0x0105)
