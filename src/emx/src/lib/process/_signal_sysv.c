/* $Id: _signal_sysv.c 2254 2005-07-17 12:25:44Z bird $ */
/** @file
 *
 * LIBC - System V style signal().
 *
 ...
 */
void (*_signal_sysv(int iSignalNo, void (*pfnHandler)(int)))(int)
{
    ...
    {
        errno = EINVAL;
        LIBCLOG_ERROR_RETURN_MSG(SIG_ERR, "ret SIG_ERR - SIG_ACK is not supported by System V style signal handling!\n");
    }
    if (pfnHandler == SIG_HOLD)
    {
        errno = EINVAL;
        LIBCLOG_ERROR_RETURN_MSG(SIG_ERR, "ret SIG_ERR - SIG_HOLD is not supported by System V style signal handling!\n");
    }
    if (!__SIGSET_SIG_VALID(iSignalNo))
    {
        errno = EINVAL;
        LIBCLOG_ERROR_RETURN_MSG(SIG_ERR, "ret SIG_ERR - Invalid signal number %d\n", iSignalNo);
    }
