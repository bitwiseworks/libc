...
#define CHILD "spawnvpe-3-child.cmd"

#define ENVC    20
#define ENVLEN  (4 * 1020)

// Note: The total cmd.exe command line is limited to 1024 chars
#define ARGC    20
#define ARGLEN  50
...
int main (void)
{
  ...
  for (i = 1; i <= ARGC; ++ i)
  {
    ...
    // Note: cmd.exe gets screwed by args containing some punctuation, limit to digits
    memset (argv [i], '0' + i % 10, ARGLEN);
  }
  argv [ARGC + 1] = NULL;
