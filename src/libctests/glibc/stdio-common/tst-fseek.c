...
#else
  sprintf (buf, "%s/tst-fseek.XXXXXX", tmpdir);
  fname = strdup(buf);
#endif
  if (fname == NULL)
...
#ifndef __EMX__ /* TODO: figure out what the hell they are getting at here. There's not writing or
                         anything which should case any of those two to be changed... */
