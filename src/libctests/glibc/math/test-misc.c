...
# endif
  ...
  /* This is a test for the strange long doubles in x86 FPUs.  */
  {
    ...
    if (fpclassify (u.d) != FP_NORMAL)
      {
        if (    fpclassify (u.d) == FP_SUBNORMAL
            &&  !isnormal(u.d) && !isnan(u.d) && !isinf(u.d) && isfinite(u.d))
          printf ("warning: fpclassify (0x00008000000000000000) failed: FP_SUBNORMAL (%Lg) - TODO investigate this on BSD/LIBC!!!\n", u.d);
        else
          {
            printf ("fpclassify (0x00008000000000000000) failed: %d (%Lg) isnormal=%d isnan=%d isinf=%d isfinite=%d\n",
                    fpclassify (u.d), u.d, isnormal(u.d), isnan(u.d), isinf(u.d), isfinite(u.d));
            result = 1;
          }
      }
  }
