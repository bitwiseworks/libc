...
waitpid WCONTINUED on continued returned %d != %d (status %x)\n",
      ...
      RETURN (EXIT_FAILURE);
	   ...
	   || !WIFCONTINUED (fail))
    {
      printf ("waitpid WCONTINUED on continued: status %x (stopped=%d,signaled=%d,exited=%d,continued=%d)\n",
              fail, WIFSTOPPED (fail), WIFSIGNALED (fail), WIFEXITED (fail), WIFCONTINUED (fail));
      RETURN (EXIT_FAILURE);
    }
#endif
