...
#define WSTRTEST(name, exp) \
...
#ifdef HAVE_NL_LANGINFO_EXTENTIONS
  /* Extensions.  */
  ...
  STRTEST (NEGATIVE_SIGN, "");
  STRTEST (GROUPING, "");
#endif /* HAVE_NL_LANGINFO_EXTENTIONS */
...
do_test (void)
{
  ...
  /* Replace the locale, set it back to C and retry.  */
  if (   setlocale (LC_ALL, "jp_JP") != NULL
      || setlocale (LC_ALL, "jp") != NULL
      || setlocale (LC_ALL, "de_DE") != NULL
      || setlocale (LC_ALL, "no_NO") != NULL
      || setlocale (LC_ALL, "en_US") != NULL
      || setlocale (LC_ALL, "en") != NULL
      )
    {
      /* First use the name "C".  */
      if (setlocale (LC_ALL, "C") == NULL)
      ...
      if (setlocale (LC_ALL, "POSIX") == NULL)
        ...
        result |= run_test ("Retest POSIX");
    }
  else
    puts ("Skipping last test since we fail to find a valid locale.");
