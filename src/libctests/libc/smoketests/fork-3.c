...
/** @file
 *
 * Fork + dlopen/dlclose/dlsym testcase, tickets #73, #103 and #114.
 *
...
int main()
{
    ...
    /*
     * Zero test: check that fork works.
     */
    if (forkit("zero fork"))
        return failure("FAILURE");

    /*
     * First test: load and unload a dll and then fork.
     */
    printf("fork-3: load and unload and then fork...\n");
    void *pvDll = dlopen(szDll, 0);
    if (!pvDll)
        return failure("Can't open '%s': %s\n", szDll, dlerror());
    if (dlclose(pvDll))
        return failure("Can't close '%s': %s\n", szDll, dlerror());
    if (forkit("first fork"))
        return failure("FAILURE");

    /*
     * Second test: load dll and fork.
     */
    printf("fork-3: load and fork...\n");
    pvDll = dlopen(szDll, 0);
    if (!pvDll)
        return failure("Can't reopen '%s': %s\n", szDll, dlerror());
    if (forkit("second fork"))
        return failure("FAILURE");
